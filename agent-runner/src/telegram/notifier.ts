...
export const createNotifier = (config: NotifierConfig): Notifier => {
  ...
  const editMessage = async (messageId: number, text: string): Promise<boolean> => {
    const res = await fetch(`${TELEGRAM_API}/bot${config.botToken}/editMessageText`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        chat_id: config.chatId,
  ...
  return {
    ...
    agentProgress: async (messageId: number, text: string): Promise<boolean> => {
