...
export const createAgentManager = (deps: ManagerDeps) => {
  ...
  const checkBudget = (): boolean => {
    ...
    return dailySpendUsd + deps.config.agent.maxBudgetPerTask <= deps.config.agent.maxDailyBudget;
  };
  ...
  const spawnAgent = async (task: AgentTask, retryCount: number = 0): Promise<SpawnResult> => {
    ...
    try {
      ...
      if (isResuming) {
        console.log(`Resuming work on ${taskSlug} (branch ${branchName} already exists)`);
        ...
        const [gitLog, gitDiff] = await Promise.all([
          getCommitLog(workingDir, `origin/${projectConfig.defaultBranch}`, "HEAD"),
    ...
    const skillsSection = skills.length > 0 ? formatSkillsCatalog(skills) : undefined;
    ...
    runAgent(
      ...
      .catch(async (err) => {
        ...
        deps.onAgentDone(task, { costUsd: 0, crashed: true, error: errMsg }, retryCount);
      });
  ...
  const getState = () => ({
    activeAgents: Object.fromEntries(activeAgents) as Record<string, ActiveAgent>,
    dailySpendUsd,
    dailySpendDate,
  });
